...
//console.log(time);
var name = 'listings-lvl'+currentLevel+'-'+data.directories[dirIndex].directoryName+'-'+currentIndex+'-'+time;  //!!!!!
...
function handleData(error, response, body){
	...
	if(currentIndex < data.directories[dirIndex].listings.length){//if we still have indexes to find, keep scraping
		...
		console.log('adding directory #'+currentIndex+'-'+parentDirectoryURL);
		addDirectory(parentDirectoryURL);
	}else{ //else write to database
		writeToDatabase();
		console.log('successfully written to database #',dirIndex);
		dirIndex++;
		if(dirIndex < data.directories.length){
			currentIndex = 0; 
			parentDirectoryURL = data.directories[dirIndex].listings[currentIndex].url;
			name = 'listings-lvl'+currentLevel+'-'+data.directories[dirIndex].directoryName+'-'+currentIndex+'-'+time;
			db = useDatabase(name);
			addDirectory(parentDirectoryURL);
			console.log('finished writing to database, moving on to #'+dirIndex);
		} else{
			console.log('done!');
		}
	}		
